...
pdf("treshold_exprssion_Control.pdf")
hist(log(rowMeans(Control_data_matrix)), 100)
dev.off()
REF <- rownames(Control_data_matrix)[log(rowMeans(Control_data_matrix))>5]
length(REF)
save(REF, file="Control_REF.Rdata")
REF_db <- as.character(db$GeneID[na.omit(match(REF, toupper(db$Symbol)))])
...
# pho_KEGGresult<-find_enriched_pathway(genes,species='hsa', download_latest = T)
...
KEGG_HCV.NEW <- find_enriched_pathway(HCV.NEW_db, species = "hsa", returned_adjpvalue = 0.0007, refGene=union(REF_db,HCV.NEW_db), download_latest=T)[[1]][,c(1,6)]
KEGG_HCV.NEW_diff_pathways <- as.character(KEGG_HCV.NEW$Pathway_Name)
KEGG_HCV.NEW_all_pathways <- find_enriched_pathway(HCV.NEW_db, returned_pvalue=1, returned_adjpvalue = 1, refGene=union(REF_db,HCV.NEW_db), species = "hsa",download_latest=T)[[1]][,c(1,6)]

SARS_db <- as.character(db$GeneID[na.omit(match(SARS_DE, toupper(db$Symbol)))])
KEGG_SARS <- find_enriched_pathway(SARS_db, species = "hsa",returned_adjpvalue = 0.0007, refGene=union(REF_db,SARS_db), download_latest=T)[[1]][,c(1,6)]
KEGG_SARS_diff_pathways <- as.character(KEGG_SARS$Pathway_Name)
KEGG_SARS_all_pathways <- find_enriched_pathway(SARS_db, returned_pvalue=1, returned_adjpvalue = 1, refGene=union(REF_db,SARS_db), species = "hsa",download_latest=T)[[1]][,c(1,6)]

SARS.HCV_db <- as.character(db$GeneID[na.omit(match(SARS.HCV_DE, toupper(db$Symbol)))])
KEGG_SARS.HCV <- find_enriched_pathway(SARS.HCV_db, species = "hsa",returned_adjpvalue = 0.0007, refGene=union(REF_db,SARS.HCV_db), download_latest=T)[[1]][,c(1,6)]
...
KEGG_SARS.HCV_all_pathways <- find_enriched_pathway(SARS.HCV_db, returned_pvalue=1, returned_adjpvalue = 1, species = "hsa", refGene=union(REF_db,SARS.HCV_db), download_latest=T)[[1]][,c(1,6)]
##################################################################################

#PLOT unique/union DE pathways
Unique_DE_pathways <- unique(c(KEGG_HCV.NEW_diff_pathways, KEGG_SARS_diff_pathways, KEGG_SARS.HCV_diff_pathways))[-12]
...
merged_pathway_df[merged_pathway_df==Inf] <- 8.8#max non-infinite value
...
pdf("Unique_DE_pathways_paper.pdf", height = 14, width = 14)
...
write.xlsx(HCV_pathways_total, file="HCV_pathways_total_REF.xlsx")
...
write.xlsx(SARS_pathways_total, file="SARS_pathways_total_REF.xlsx")
...
write.xlsx(SARS.HCV_pathways_total, file="SARS.HCV_pathways_total_REF.xlsx")
